...
class StlMeshParserBoundsTest {
    ...
    @Test
    fun parseForDisplayCapsLargeBinaryMeshButPreservesSourceBounds() {
        val file = File.createTempFile("mobileslicer-display-lod-", ".stl")
        try {
            file.writeBytes(binaryStl(triangleCount = 5))

            val prepared = StlMeshParser.parseForDisplay(file, maxDisplayTriangles = 2)

            assertTrue(prepared.reducedForDisplay)
