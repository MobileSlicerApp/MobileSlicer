...
5. Keep `scripts/verify_android.sh orca-step-project-reference-probe` green until a real desktop-Orca STEP project reference path exists. Do not replace that with a pre-converted STL fixture.
6. Keep `scripts/verify_android.sh orca-step-sliced-source-fixture` and `scripts/verify_android.sh orca-step-multi-plate-sliced-source-fixture` green so MobileSlicer-produced STEP packages continue to preserve part-level `.step`/`.stp` `source_file` evidence across single-plate and multi-plate sliced exports.
7. Keep `./gradlew testDebugUnitTest`, `./gradlew assembleDebug`, `git diff --check`, `scripts/verify_android.sh object-process <device>`, `scripts/verify_android.sh orca-object-label-parity <device>`, `scripts/verify_android.sh orca-import-smoke <device>`, and `scripts/verify_android.sh orca-project-parity-device-matrix <device>` green before pushing to a device.
