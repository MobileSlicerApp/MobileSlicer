...
SCENARIO("Placeholder parser scripting", "[PlaceholderParser]") {
    ...
    SECTION("boolean expression parser: bang before variable") { REQUIRE(boolean_expression("!false")); }
    SECTION("boolean expression parser: bang before parenthesized expression") { REQUIRE(boolean_expression("!(13 == 14)")); }
    SECTION("boolean expression parser: H2D-style timelapse guard") {
        PlaceholderParser timelapse_parser;
        auto timelapse_config = DynamicPrintConfig::full_print_config();
        timelapse_config.set_key_value("spiral_mode", new ConfigOptionBool(false));
        timelapse_config.set_key_value("has_timelapse_safe_pos", new ConfigOptionBool(false));
        timelapse_config.set_key_value("timelapse_type", new ConfigOptionInt(0));
        REQUIRE(timelapse_parser.process("{if !spiral_mode && !(has_timelapse_safe_pos && timelapse_type == 0)}OK{endif}", 0, &timelapse_config, nullptr, nullptr) == "OK");
    }
...
SCENARIO("Placeholder parser variables", "[PlaceholderParser]") {
    ...
    SECTION("Prusa XL inline elsif pressure advance gcode") {
        auto xl_config = DynamicPrintConfig::full_print_config();
        xl_config.set_deserialize_strict({
            { "printer_notes", "PRINTER_VENDOR_PRUSA3D PRINTER_MODEL_XLIS" },
            { "nozzle_diameter", "0.6;0.6;0.6;0.6;0.6" }
        });
        const std::string pa_line =
            ...
            "{endif}";
        REQUIRE(parser.process(pa_line, 0, &xl_config, nullptr, nullptr) == "\nM572 S0.02 ; Filament gcode\n");
    }
}
