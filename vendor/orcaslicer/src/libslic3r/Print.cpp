...
std::string Print::export_gcode(const std::string& path_template, GCodeProcessorResult* result, ThumbnailsGeneratorCallback thumbnail_cb)
{
...
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative", "print_export_gcode: output_filepath failed: %s", e.what());
#endif
        throw;
    }
#ifdef __ANDROID__
    android_log_export("path", path.c_str());
#endif
    std::string message;
    ...
    GCode gcode;
#ifdef __ANDROID__
    android_log_export("gcode_created");
#endif
    ...
    try {
#ifdef __ANDROID__
        android_log_export("do_export_begin");
#endif
        gcode.do_export(this, path.c_str(), result, thumbnail_cb);
#ifdef __ANDROID__
        android_log_export("do_export_done");
#endif
    } catch (const std::exception &e) {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative", "print_export_gcode: do_export failed: %s", e.what());
#endif
        throw;
    }
    try {
#ifdef __ANDROID__
        android_log_export("export_layer_filaments_begin");
#endif
        gcode.export_layer_filaments(result);
#ifdef __ANDROID__
        android_log_export("export_layer_filaments_done");
#endif
    } catch (const std::exception &e) {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative", "print_export_gcode: export_layer_filaments failed: %s", e.what());
#endif
        throw;
    }
    //BBS
...
Points Print::first_layer_wipe_tower_corners(bool check_wipe_tower_existance) const
{
        ...
        return corners;
    {
        ...
        const double brim_margin = m_wipe_tower_data.brim_width + 2.0;
        const bool bbx_is_already_positioned =
            pt0.x() >= wipe_tower_pos.x() - brim_margin &&
            pt0.y() >= wipe_tower_pos.y() - brim_margin &&
            pt0.x() + width <= wipe_tower_pos.x() + m_config.prime_tower_width.value + brim_margin &&
            pt0.y() + depth <= wipe_tower_pos.y() + m_wipe_tower_data.depth + brim_margin;
        ...
        for (Vec2d& pt : pts) {
            ...
            // Orca normally stores the wipe-tower bbox in local tower coordinates
            // and offsets it here. Android's real Type1 tower path may already
            // store a positioned bbox; offsetting that again makes the skirt hull
            // wrap a phantom tower outside the printable area.
            if (!bbx_is_already_positioned) {
                pt += wipe_tower_pos;
            }
...
std::vector<Polygons> Print::get_extruder_printable_polygons() const
{
    ...
    const size_t                    extruder_count = std::max<size_t>(1, m_config.nozzle_diameter.values.size());
    const Polygons                  bed_printable_area{Model::getBedPolygon()};
    for (const auto &e_printable_area : extruder_printable_areas) {
        Polygons polys;
        if (e_printable_area.size() >= 3) {
            polys = {Polygon::new_scale(e_printable_area)};
        }
        if (polys.empty() || polys.front().points.size() < 3) {
            polys = bed_printable_area;
        }
        extruder_printable_polys.emplace_back(std::move(polys));
    }
    if (extruder_printable_polys.size() < extruder_count) {
        extruder_printable_polys.resize(extruder_count, bed_printable_area);
    }
...
std::vector<Polygons> Print::get_extruder_unprintable_polygons() const
{
    ...
    for (const auto &e_printable_area : extruder_printable_areas) {
        Polygons polys;
        if (e_printable_area.size() >= 3) {
            polys = diff(printable_poly, Polygon::new_scale(e_printable_area));
        }
        extruder_unprintable_polys.emplace_back(std::move(polys));
    }
    if (extruder_unprintable_polys.size() < extruder_count) {
        extruder_unprintable_polys.resize(extruder_count);
    }
...
void Print::_make_wipe_tower()
{
...
#ifdef __ANDROID__
    {
        std::string layers_debug;
        size_t printed = 0;
        for (const LayerTools &lt : m_wipe_tower_data.tool_ordering.layer_tools()) {
            if (printed++ >= 8)
                break;
            layers_debug += " z=" + std::to_string(lt.print_z)
                         + " hwt=" + std::to_string(lt.has_wipe_tower ? 1 : 0)
                         + " parts=" + std::to_string(lt.wipe_tower_partitions)
    ...
    if (!is_wipe_tower_type2) {
...
#ifdef __ANDROID__
        {
            size_t total_tool_changes = 0;
            std::string layer_sizes;
            for (size_t i = 0; i < m_wipe_tower_data.tool_changes.size(); ++i) {
                total_tool_changes += m_wipe_tower_data.tool_changes[i].size();
                if (i < 8) {
        ...
        m_wipe_tower_data.bbx = BoundingBoxf(
            Vec2d(double(wipe_tower.position().x()) - double(m_wipe_tower_data.brim_width),
                  double(wipe_tower.position().y()) - double(m_wipe_tower_data.brim_width)),
            Vec2d(double(wipe_tower.position().x()) + double(wipe_tower.width()) + double(m_wipe_tower_data.brim_width),
                  double(wipe_tower.position().y()) + double(m_wipe_tower_data.depth) + double(m_wipe_tower_data.brim_width)));
#endif
