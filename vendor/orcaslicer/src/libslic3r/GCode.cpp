...
static void orca_set_custom_gcode_timelapse_context(
    ...
    int most_used_physical_extruder_id = 0,
    int curr_physical_extruder_id = 0,
    const Point *timelapse_pos = nullptr)
{
    config.set_key_value("timelapse_type", new ConfigOptionInt(static_cast<int>(print_config.timelapse_type.value)));
    config.set_key_value("spiral_mode", new ConfigOptionBool(print_config.spiral_mode.value));
    config.set_key_value("most_used_physical_extruder_id", new ConfigOptionInt(most_used_physical_extruder_id));
    config.set_key_value("curr_physical_extruder_id", new ConfigOptionInt(curr_physical_extruder_id));
    config.set_key_value("timelapse_pos_x", new ConfigOptionInt(timelapse_pos ? int(timelapse_pos->x()) : int(DefaultTimelapsePos.x())));
    config.set_key_value("timelapse_pos_y", new ConfigOptionInt(timelapse_pos ? int(timelapse_pos->y()) : int(DefaultTimelapsePos.y())));
    config.set_key_value("has_timelapse_safe_pos", new ConfigOptionBool(timelapse_pos != nullptr && *timelapse_pos != DefaultTimelapsePos));
}
...
namespace DoExport {
		...
		PrintStatistics 		    &print_statistics)
    {
	    ...
	    if (! extruders.empty()) {
            ...
            auto wipe_tower_filament_for = [&wipe_tower_data](unsigned int extruder_id) {
                return extruder_id < wipe_tower_data.used_filament.size() ? wipe_tower_data.used_filament[extruder_id] : 0.f;
            };
		    for (const Extruder &extruder : extruders) {
                const double wipe_tower_filament = has_wipe_tower ? wipe_tower_filament_for(extruder.id()) : 0.f;
	            double used_filament   = extruder.used_filament() + wipe_tower_filament;
	            double extruded_volume = extruder.extruded_volume() + wipe_tower_filament * 2.4052f; // assumes 1.75mm filament diameter
	            ...
	            print_statistics.total_wipe_tower_cost += wipe_tower_filament * 2.4052f * extruder.filament_density() * 0.001 * extruder.filament_cost() * 0.001;
	        }
...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        if (is_bbl_printers && m_config.exclude_object) {
...
*/
    ...
    // Process filament-specific gcode in extruder order.
    {
        ...
        config.set_key_value("max_layer_z", new ConfigOptionFloat(m_max_layer_z));
        orca_set_custom_gcode_timelapse_context(config, print.config());
...
std::string GCode::placeholder_parser_process(const std::string &name, const std::string &templ, unsigned int current_filament_id, const DynamicConfig *config_override)
{
    ...
    {
#ifdef __ANDROID__
        std::string preview = templ.substr(0, 768);
        std::replace(preview.begin(), preview.end(), '\n', '|');
        std::replace(preview.begin(), preview.end(), '\r', '|');
        __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative",
            "placeholder_parser failed template=%s error=%s preview=%s",
            name.c_str(), err.what(), preview.c_str());
#endif
    ...
    const bool                               prime_extruder)
{
    ...
    if (!m_config.time_lapse_gcode.value.empty() && !is_BBL_Printer()) {
        ...
        config.set_key_value("layer_z", new ConfigOptionFloat(print_z));
        config.set_key_value("max_layer_z", new ConfigOptionFloat(m_max_layer_z));
        orca_set_custom_gcode_timelapse_context(
            config,
            print.config(),
            m_config.physical_extruder_map.get_at(most_used_extruder),
            m_writer.filament() ? m_config.physical_extruder_map.get_at(m_writer.filament()->extruder_id()) : 0);
    ...
    for (const LayerToPrint &layer_to_print : layers) {
        ...
        if (layer_to_print.object_layer != nullptr) {
            ...
            for (size_t region_id = 0; region_id < layer.regions().size(); ++ region_id) {
                ...
                const PrintRegion &layer_region_config = layerm->region();
                ...
                for (const ObjectByExtruder::Island::Region::Type entity_type : { ObjectByExtruder::Island::Region::INFILL, ObjectByExtruder::Island::Region::PERIMETERS }) {
                    for (const ExtrusionEntity *ee : (entity_type == ObjectByExtruder::Island::Region::INFILL) ? layerm->fills.entities : layerm->perimeters.entities) {
                        ...
                        int correct_extruder_id = layer_tools.extruder(*extrusions, layer_region_config);
                        ...
                        for (unsigned int extruder : printing_extruders)
                        {
                            ...
                            for (size_t i = 0; i <= n_slices; ++ i) {
                                ...
                                if (// extrusions->first_point does not fit inside any slice
                                    ...
                                    islands[island_idx].by_region[region.print_region_id()].print_region = &layer_region_config;
    ...
    if (layer.id() < 5) {
        ...
        for (const auto &entry : by_extruder) {
            size_t non_empty = 0;
            for (const ObjectByExtruder &object_by_extruder : entry.second)
                if (! object_by_extruder.islands.empty() || (object_by_extruder.support != nullptr && ! object_by_extruder.support->empty()))
                    ++ non_empty;
            by_extruder_debug += " e" + std::to_string(entry.first) + ":objects=" + std::to_string(non_empty);
        }
        __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
            "orca_gcode_by_extruder layer=%zu%s",
            layer.id(), by_extruder_debug.c_str());
    }
#endif
    ...
    if (layer.id() < 5) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
            "orca_gcode_filament_instances layer=%zu%s",
            layer.id(), instances_debug.c_str());
    }
#endif
    ...
    auto insert_timelapse_gcode = [this, print_z, &print, &most_used_extruder, &layer_object_label_ids,&printed_objects = std::as_const(m_printed_objects)]() -> std::string {
        ...
        if (!print.config().time_lapse_gcode.value.empty()) {
            ...
            orca_set_custom_gcode_timelapse_context(
                ...
                print.config(),
                m_config.physical_extruder_map.get_at(most_used_extruder),
                m_config.physical_extruder_map.get_at(ctx.curr_extruder_id),
                &timelapse_pos);
            auto &parser = this->placeholder_parser();
            parser.set("layer_num", m_layer_index);
            parser.set("layer_z", print_z);
            parser.set("max_layer_z", m_max_layer_z);
            parser.set("most_used_physical_extruder_id", m_config.physical_extruder_map.get_at(most_used_extruder));
            parser.set("curr_physical_extruder_id", m_config.physical_extruder_map.get_at(ctx.curr_extruder_id));
            parser.set("timelapse_pos_x", (int)timelapse_pos.x());
            parser.set("timelapse_pos_y", (int)timelapse_pos.y());
            parser.set("has_timelapse_safe_pos", timelapse_pos != DefaultTimelapsePos);
            parser.set("timelapse_type", static_cast<int>(print.config().timelapse_type.value));
            parser.set("spiral_mode", print.config().spiral_mode.value);
        ...
        if (has_wipe_tower) {
            ...
            if (layer.id() < 5) {
                ...
                __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
                    ...
                    layer.id(), extruder_id, wipe_tower_empty ? 1 : 0, finish_layer_for_tool ? 1 : 0,
                    current_filament, m_writer.need_toolchange(extruder_id) ? 1 : 0);
            }
#endif
            if (!wipe_tower_empty) {
            ...
            if (layer.id() < 5) {
                ...
                __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
                    ...
                    layer.id(), extruder_id, current_filament, m_writer.need_toolchange(extruder_id) ? 1 : 0);
            }
#endif
...
std::string GCode::extrude_perimeters(const Print &print, const std::vector<ObjectByExtruder::Island::Region> &by_region, bool is_first_layer, bool is_infill_first)
{
    ...
    for (const ObjectByExtruder::Island::Region &region : by_region)
        if (! region.perimeters.empty()) {
            m_config.apply((region.print_region != nullptr ? *region.print_region : print.get_print_region(&region - &by_region.front())).config());
