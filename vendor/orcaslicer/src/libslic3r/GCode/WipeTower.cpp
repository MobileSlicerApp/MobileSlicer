...
void WipeTower::generate_new(std::vector<std::vector<WipeTower::ToolChangeResult>> &result)
{
...
#ifdef __ANDROID__
    {
        size_t planned = 0;
        for (const auto &layer : m_plan)
            planned += layer.tool_changes.size();
        const float first_depth = (!m_plan.empty() && !m_plan.front().tool_changes.empty()) ? m_plan.front().tool_changes.front().required_depth : -1.f;
        const float first_wipe = (!m_plan.empty() && !m_plan.front().tool_changes.empty()) ? m_plan.front().tool_changes.front().wipe_volume : -1.f;
        __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
            "orca_wipe_generate_new_begin plan_layers=%zu planned=%zu first_tc=%zu first_depth=%.3f first_wipe=%.3f width=%.3f perimeter=%.3f extra=%.3f",
            m_plan.size(), planned, m_plan.front().tool_changes.size(), double(first_depth), double(first_wipe),
            double(m_wipe_tower_width), double(m_perimeter_width), double(m_extra_spacing));
    }
#endif
    //m_extra_spacing = 1.f;
...
#ifdef __ANDROID__
    {
        size_t blocks = m_wipe_tower_blocks.size();
        float first_plan_depth = !m_plan.empty() ? m_plan.front().depth : -1.f;
        float first_block_depth = !m_wipe_tower_blocks.empty() ? m_wipe_tower_blocks.front().depth : -1.f;
        __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
            "orca_wipe_generate_new_after_plan blocks=%zu first_plan_depth=%.3f first_block_depth=%.3f tower_depth=%.3f width=%.3f extra=%.3f",
            blocks, double(first_plan_depth), double(first_block_depth), double(m_wipe_tower_depth),
            double(m_wipe_tower_width), double(m_extra_spacing));
    }
#endif
