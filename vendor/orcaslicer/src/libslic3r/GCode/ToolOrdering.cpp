...
void ToolOrdering::handle_dontcare_extruder(unsigned int last_extruder_id)
{
    ...
    for (const LayerTools &lt : m_layer_tools) {
        if (! lt.has_object && ! lt.has_support)
            continue;
        if (printed ++ >= 5)
            break;
        layers_debug += " z=" + std::to_string(lt.print_z) + "[" + orca_debug_join_u(lt.extruders) + "]";
    }
    __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
        "orca_tool_after_dontcare first=%u layers=%s",
        last_extruder_id, layers_debug.c_str());
#endif
}
...
void ToolOrdering::collect_extruders(const PrintObject &object, const std::vector<std::pair<double, unsigned int>> &per_layer_extruder_switches)
{
    ...
    if (object.get_id() < 4) {
        ...
        for (const LayerTools &layer : m_layer_tools) {
            if (! layer.has_object && ! layer.has_support)
                continue;
            if (printed ++ >= 5)
                break;
            layers_debug += " z=" + std::to_string(layer.print_z) + "[" + orca_debug_join_u(layer.extruders) + "]";
        }
        __android_log_print(ANDROID_LOG_INFO, "MobileSlicerNative",
            "orca_tool_collect object=%zu layers=%s",
            object.get_id(), layers_debug.c_str());
    }
#endif
}
