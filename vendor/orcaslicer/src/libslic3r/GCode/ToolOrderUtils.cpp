...
namespace Slic3r
{
    static float flush_cost_or_zero(const std::vector<std::vector<float>>& wipe_volumes, unsigned int from, unsigned int to)
    {
        if (from >= wipe_volumes.size() || to >= wipe_volumes[from].size())
            return 0.f;
        return wipe_volumes[from][to];
    }

    static const FlushMatrix& flush_matrix_or_empty(const std::vector<FlushMatrix>& flush_matrix, size_t idx)
    {
        static const FlushMatrix empty;
        return idx < flush_matrix.size() ? flush_matrix[idx] : empty;
    }
    ...
    int MinCostMaxFlow::get_distance(int idx_in_left, int idx_in_right)
    {
        if (l_nodes[idx_in_left] == -1) {
            ...
            for (int i = 0; i < matrix.size(); ++i)
                if (idx_in_right < matrix[i].size())
                    sum += matrix[i][idx_in_right];
            sum /= matrix.size();
            return -sum;
        }

        return flush_cost_or_zero(matrix, l_nodes[idx_in_left], r_nodes[idx_in_right]);
    }
        ...
        float* min_cost)
    {
        ...
        while (idx > 0) {
            ...
            for (size_t k = 0; k < is_visited.size(); ++k) {
                if (!is_visited[k]) {
                    float candidate_cost = flush_cost_or_zero(wipe_volumes, *prev_filament, curr_layer_extruders[k]);
                    if (candidate_cost < target_cost ||
                        (candidate_cost == target_cost && prev_filament == curr_layer_extruders[k])) {
                        target_idx = k;
        ...
        float* min_cost)
    {
        ...
        do {
            ...
            for (size_t idx = 0; idx < curr_layer_extruders.size(); ++idx) {
                if (prev_extruder_1)
                    curr_layer_cost += flush_cost_or_zero(wipe_volumes, *prev_extruder_1, curr_layer_extruders[idx]);
            ...
            do {
                ...
                for (size_t idx = 0; idx < next_layer_extruders.size(); ++idx) {
                    if (prev_extruder_2)
                        total_cost += flush_cost_or_zero(wipe_volumes, *prev_extruder_2, next_layer_extruders[idx]);
        ...
        if (min_cost) {
            ...
            for (size_t idx = 0; idx < best_seq.size(); ++idx) {
                if (prev_extruder)
                    real_cost += flush_cost_or_zero(wipe_volumes, *prev_extruder, best_seq[idx]);
                    ...
                    if (state >> target & 1) {
                        for (unsigned int mid_point = 0; mid_point < all_extruders.size(); ++mid_point) {
                            if (state >> mid_point & 1) {
                                auto tmp = cache[state - (1 << target)][mid_point] + flush_cost_or_zero(wipe_volumes, all_extruders[mid_point], all_extruders[target]);
        ...
        float* cost)
    {
        ...
        if (curr_layer_extruders.size() == 1) {
            if (cost) {
                *cost = 0;
                if (start_extruder_id)
                    *cost = flush_cost_or_zero(wipe_volumes, *start_extruder_id, curr_layer_extruders[0]);
            }
            return curr_layer_extruders;
        }
        ...
        std::vector<std::vector<unsigned int>>* filament_sequences)
    {
        ...
        for (size_t idx = 0; idx < groups.size(); ++idx) {
            ...
            if (groups[idx].empty())
                continue;
            const auto& group_flush_matrix = flush_matrix_or_empty(flush_matrix, idx);
            ...
            for (size_t layer = 0; layer < layer_filaments.size(); ++layer) {
                ...
                if (auto iter = custom_layer_sequence_map.find(layer); iter != custom_layer_sequence_map.end()) {
                    ...
                    for (auto& f : sequence_in_group) {
                        if (prev) { tmp_cost += flush_cost_or_zero(group_flush_matrix, *prev, f); }
                        prev = f;
                    }
                    cost += tmp_cost;
                ...
                if (auto iter = caches.find(hash_key); iter != caches.end()) {
                    ...
                    sequence = get_extruders_order(group_flush_matrix, filament_used_in_group, filament_used_in_group_next_layer, current_extruder_id, use_forcast, &tmp_cost);
