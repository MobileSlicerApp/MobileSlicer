...
#ifdef __ANDROID__
static void mobile_slicer_support_log(const std::string &message)
{
    __android_log_write(ANDROID_LOG_INFO, "MobileSlicerSupport", message.c_str());
}
#else
static void mobile_slicer_support_log(const std::string &) {}
#endif
...
static void install_support_layers_fallback(
    PrintObject &object,
    ...
    const SupportGeneratorLayersPtr &base_interface_layers)
{
    ...
    layers_sorted.reserve(
        raft_layers.size() + bottom_contacts.size() + top_contacts.size() +
        ...
        if (! empty) {
            ...
            if (this_layer_contacts_only) {
                for (auto it = object.support_layers().rbegin(); it != object.support_layers().rend(); ++it) {
                    const SupportLayer &other_layer = **it;
                    if (std::abs(other_layer.print_z - top_contact_bottom_z) < EPSILON) {
...
void PrintObjectSupportMaterial::generate(PrintObject &object)
{
    ...
    const auto nonempty_layer_count = [](const SupportGeneratorLayersPtr &layers) {
        size_t count = 0;
    ...
    {
        size_t nonempty_top_contacts = 0;
        for (const SupportGeneratorLayer *layer : top_contacts) {
            if (layer != nullptr && ! layer->polygons.empty()) {
                ++nonempty_top_contacts;
            }
        }
        std::ostringstream message;
        message
            << "support_generate top_contacts size=" << top_contacts.size()
            << " nonempty=" << nonempty_top_contacts
            << " layer_count=" << object.layer_count()
            << " enable_support=" << (m_object_config->enable_support.value ? 1 : 0)
            << " support_type=" << static_cast<int>(m_object_config->support_type.value)
            << " threshold=" << m_object_config->support_threshold_angle.value
            << " max_bridge_length=" << m_object_config->max_bridge_length.value
            << " object_raft_layers=" << m_object_config->raft_layers.value
            << " slicing_raft_layers=" << m_slicing_params.raft_layers()
            << " slicing_base_raft_layers=" << m_slicing_params.base_raft_layers
            << " slicing_interface_raft_layers=" << m_slicing_params.interface_raft_layers;
        mobile_slicer_support_log(message.str());
    }
        ...
        layer_storage, layer_support_areas);
    {
        std::ostringstream message;
        message
            << "support_generate bottom_contacts size=" << bottom_contacts.size()
            << " nonempty=" << nonempty_layer_count(bottom_contacts);
        mobile_slicer_support_log(message.str());
    }
        ...
        object, bottom_contacts, top_contacts, layer_storage);
    {
        std::ostringstream message;
        message
            << "support_generate intermediate_layers size=" << intermediate_layers.size()
            << " nonempty=" << nonempty_layer_count(intermediate_layers);
        mobile_slicer_support_log(message.str());
    }
    ...
    this->trim_support_layers_by_object(object, top_contacts, m_slicing_params.gap_support_object, m_slicing_params.gap_object_support, m_support_params.gap_xy);
    {
        std::ostringstream message;
        message
            << "support_generate top_contacts_after_trim size=" << top_contacts.size()
            << " nonempty=" << nonempty_layer_count(top_contacts);
        mobile_slicer_support_log(message.str());
    }
    ...
    this->generate_base_layers(object, bottom_contacts, top_contacts, intermediate_layers, layer_support_areas);
    {
        std::ostringstream message;
        message
            << "support_generate intermediate_layers_after_base size=" << intermediate_layers.size()
            << " nonempty=" << nonempty_layer_count(intermediate_layers);
        mobile_slicer_support_log(message.str());
    }
    ...
    this->trim_top_contacts_by_bottom_contacts(object, bottom_contacts, top_contacts);
    {
        std::ostringstream message;
        message
            << "support_generate top_contacts_after_bottom_trim size=" << top_contacts.size()
            << " nonempty=" << nonempty_layer_count(top_contacts);
        mobile_slicer_support_log(message.str());
    }
    ...
    SupportGeneratorLayersPtr raft_layers = generate_raft_base(object, m_support_params, m_slicing_params, top_contacts, interface_layers, base_interface_layers, intermediate_layers, layer_storage);
    {
        std::ostringstream message;
        message
            << "support_generate interfaces top=" << interface_layers.size()
            << " top_nonempty=" << nonempty_layer_count(interface_layers)
            << " base=" << base_interface_layers.size()
            << " base_nonempty=" << nonempty_layer_count(base_interface_layers)
            << " raft=" << raft_layers.size()
            << " raft_nonempty=" << nonempty_layer_count(raft_layers);
        mobile_slicer_support_log(message.str());
    }
...
//    interface_layers.clear();
    ...
    if (object.support_layers().empty()) {
        const size_t combined_nonempty =
            nonempty_layer_count(raft_layers) +
            nonempty_layer_count(bottom_contacts) +
            nonempty_layer_count(top_contacts) +
            nonempty_layer_count(intermediate_layers) +
            nonempty_layer_count(interface_layers) +
            nonempty_layer_count(base_interface_layers);
        if (combined_nonempty > 0) {
            mobile_slicer_support_log("support_generate fallback_install_support_layers=1");
            install_support_layers_fallback(object, raft_layers, bottom_contacts, top_contacts, intermediate_layers, interface_layers, base_interface_layers);
        }
    }
    {
        size_t nonempty_support_layers = 0;
        for (const SupportLayer *layer : object.support_layers()) {
            if (layer != nullptr) {
                ++nonempty_support_layers;
            }
        }
        std::ostringstream message;
        message
            << "support_generate support_layers_after_generate size=" << object.support_layers().size()
            << " nonnull=" << nonempty_support_layers;
        mobile_slicer_support_log(message.str());
    }
    ...
    // Isolate toolpath generation from unexpected mutation of the owning vector container.
    // The copied pointer list still targets the same SupportLayer instances on the PrintObject.
    SupportLayerPtrs support_layers_for_toolpaths = object.support_layers();
    generate_support_toolpaths(support_layers_for_toolpaths, *m_object_config, m_support_params, m_slicing_params, raft_layers, bottom_contacts, top_contacts, intermediate_layers, interface_layers, base_interface_layers);
    {
        size_t layers_with_entities = 0;
        size_t total_entity_count = 0;
        size_t total_island_count = 0;
        for (const SupportLayer *layer : object.support_layers()) {
            if (layer == nullptr) {
                continue;
            }
            if (!layer->support_fills.entities.empty()) {
                ++layers_with_entities;
                total_entity_count += layer->support_fills.entities.size();
            }
            total_island_count += layer->support_islands.size();
        }
        std::ostringstream message;
        message
            << "support_generate toolpaths layers=" << object.support_layers().size()
            << " layers_with_entities=" << layers_with_entities
            << " total_entities=" << total_entity_count
            << " total_islands=" << total_island_count;
        mobile_slicer_support_log(message.str());
    }
