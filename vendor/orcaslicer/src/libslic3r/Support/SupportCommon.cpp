...
#ifdef __ANDROID__
static void mobile_slicer_support_common_log(const std::string &message)
{
    __android_log_write(ANDROID_LOG_INFO, "MobileSlicerSupport", message.c_str());
}
#else
static void mobile_slicer_support_common_log(const std::string &) {}
#endif

static double mobile_slicer_support_area(const Polygons &polygons)
{
    double area = 0.0;
    for (const Polygon &polygon : polygons)
        area += std::abs(polygon.area());
    return area;
}

static const char *mobile_slicer_support_layer_type_name(SupporLayerType layer_type)
{
    switch (layer_type) {
    case SupporLayerType::Unknown: return "Unknown";
...
static size_t mobile_slicer_support_entity_count(const ExtrusionEntitiesPtr &entities)
{
    ...
    const SupportGeneratorLayersPtr     &base_interface_layers)
{
    ...
    layers_sorted.erase(std::unique(layers_sorted.begin(), layers_sorted.end()), layers_sorted.end());
    {
        ...
        message << "generate_support_layers merged_count=" << layers_sorted.size();
        mobile_slicer_support_common_log(message.str());
    }
    ...
    for (size_t i = 0; i < layers_sorted.size();) {
        ...
        if (! empty) {
            ...
            object.add_support_layer(layer_id ++, this_layer_id_interface, height_min, zavg);
            {
                std::ostringstream message;
                message
                    << "generate_support_layers add print_z=" << zavg
                    << " height=" << height_min
                    << " interfaces=" << num_interfaces
                    << " top_contacts=" << num_top_contacts
                    << " support_layers_now=" << object.support_layers().size();
                mobile_slicer_support_common_log(message.str());
            }
...
{
    {
        std::ostringstream message;
        message
            << "generate_support_toolpaths start layers=" << support_layers.size()
            << " raft=" << raft_layers.size()
            << " bottom=" << bottom_contacts.size()
            << " top=" << top_contacts.size()
            << " intermediate=" << intermediate_layers.size()
            << " interface=" << interface_layers.size()
            << " base_interface=" << base_interface_layers.size();
        mobile_slicer_support_common_log(message.str());
    }
...
//    const coordf_t link_max_length_factor = 3.;
    ...
    {
        std::ostringstream message;
        message << "generate_support_toolpaths after_raft layers=" << support_layers.size();
        mobile_slicer_support_common_log(message.str());
    }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id)
        {
            ...
            auto log_source = [&](const char *label, const SupportGeneratorLayerExtruded &layer_ex) {
                std::ostringstream message;
                message
                    << "support_toolpaths layer=" << support_layer_id
                    << " print_z=" << support_layer.print_z
                    << " source=" << label;
                if (layer_ex.layer == nullptr) {
                    message << " assigned=0";
                } else {
                    message
                        << " assigned=1"
                        << " src_print_z=" << layer_ex.layer->print_z
                        << " src_height=" << layer_ex.layer->height
                        << " src_type=" << mobile_slicer_support_layer_type_name(layer_ex.layer->layer_type)
                        << " polys=" << layer_ex.layer->polygons.size()
                        << " area=" << mobile_slicer_support_area(layer_ex.layer->polygons)
                        << " extrude_polys=" << layer_ex.polygons_to_extrude().size()
                        << " extrude_area=" << mobile_slicer_support_area(layer_ex.polygons_to_extrude());
                    if (layer_ex.layer->contact_polygons)
                        message << " contact_polys=" << layer_ex.layer->contact_polygons->size();
                    if (layer_ex.layer->overhang_polygons)
                        message << " overhang_polys=" << layer_ex.layer->overhang_polygons->size();
                }
                mobile_slicer_support_common_log(message.str());
            };
            log_source("bottom_contact_premerge", bottom_contact_layer);
            log_source("top_contact_premerge", top_contact_layer);
            log_source("interface_premerge", interface_layer);
            log_source("base_interface_premerge", base_interface_layer);
            log_source("base_premerge", base_layer);
            ...
            log_source("bottom_contact_postmerge", bottom_contact_layer);
            log_source("top_contact_postmerge", top_contact_layer);
            log_source("interface_postmerge", interface_layer);
            log_source("base_interface_postmerge", base_interface_layer);
            log_source("base_postmerge", base_layer);
            ...
            auto extrude_interface = [&](SupportGeneratorLayerExtruded &layer_ex, InterfaceLayerType interface_layer_type) {
                if (! layer_ex.empty() && ! layer_ex.polygons_to_extrude().empty()) {
                    ...
                    const size_t entities_before = mobile_slicer_support_entity_count(layer_ex.extrusions);
                    const size_t polygons_before = layer_ex.polygons_to_extrude().size();
                    const double area_before = mobile_slicer_support_area(layer_ex.polygons_to_extrude());
                    ...
                    message
                        ...
                        << " print_z=" << support_layer.print_z
                        << " extrude_interface="
                        << (interface_layer_type == InterfaceLayerType::TopContact ? "TopContact" :
                            interface_layer_type == InterfaceLayerType::BottomContact ? "BottomContact" :
                            interface_layer_type == InterfaceLayerType::RaftContact ? "RaftContact" :
                            interface_layer_type == InterfaceLayerType::Interface ? "Interface" : "InterfaceAsBase")
                        << " polys_before=" << polygons_before
                        << " area_before=" << area_before
                        << " entities_added=" << (mobile_slicer_support_entity_count(layer_ex.extrusions) - entities_before)
                        << " entities_total=" << mobile_slicer_support_entity_count(layer_ex.extrusions);
                    mobile_slicer_support_common_log(message.str());
                }
            };
            ...
            if ( ! base_interface_layer.empty() && ! base_interface_layer.polygons_to_extrude().empty()) {
                const size_t entities_before = mobile_slicer_support_entity_count(base_interface_layer.extrusions);
                const size_t polygons_before = base_interface_layer.polygons_to_extrude().size();
                const double area_before = mobile_slicer_support_area(base_interface_layer.polygons_to_extrude());
                ...
                message
                    << "support_toolpaths layer=" << support_layer_id
                    << " print_z=" << support_layer.print_z
                    << " extrude_base_interface polys_before=" << polygons_before
                    << " area_before=" << area_before
                    << " entities_added=" << (mobile_slicer_support_entity_count(base_interface_layer.extrusions) - entities_before)
                    << " entities_total=" << mobile_slicer_support_entity_count(base_interface_layer.extrusions);
                mobile_slicer_support_common_log(message.str());
            }

            // Base support or flange.
            if (! base_layer.empty() && ! base_layer.polygons_to_extrude().empty()) {
                const size_t entities_before = mobile_slicer_support_entity_count(base_layer.extrusions);
                const size_t polygons_before = base_layer.polygons_to_extrude().size();
                const double area_before = mobile_slicer_support_area(base_layer.polygons_to_extrude());
                Fill             *filler          = filler_support.get();
                ...
                message
                    << "support_toolpaths layer=" << support_layer_id
                    << " print_z=" << support_layer.print_z
                    << " extrude_base polys_before=" << polygons_before
                    << " area_before=" << area_before
                    << " entities_added=" << (mobile_slicer_support_entity_count(base_layer.extrusions) - entities_before)
                    << " entities_total=" << mobile_slicer_support_entity_count(base_layer.extrusions);
                mobile_slicer_support_common_log(message.str());
            }
            ...
            }
            {
                ...
                message
                    << "support_toolpaths layer=" << support_layer_id
                    << " print_z=" << support_layer.print_z
                    << " cache_nonempty=" << layer_cache.nonempty.size()
                    << " support_islands=" << support_layer.support_islands.size()
                    << " support_fill_entities_preappend=" << support_layer.support_fills.entities.size();
                mobile_slicer_support_common_log(message.str());
            }
    ...
    {
        std::ostringstream message;
        message << "generate_support_toolpaths after_prepare layers=" << support_layers.size();
        mobile_slicer_support_common_log(message.str());
    }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id) {
            ...
            }
            {
                std::ostringstream message;
                message
                    << "support_toolpaths append layer=" << support_layer_id
                    << " print_z=" << support_layer.print_z
                    << " support_fill_entities=" << support_layer.support_fills.entities.size()
                    << " support_islands=" << support_layer.support_islands.size();
                mobile_slicer_support_common_log(message.str());
            }
    ...
    {
        std::ostringstream message;
        message << "generate_support_toolpaths end layers=" << support_layers.size();
        mobile_slicer_support_common_log(message.str());
    }
