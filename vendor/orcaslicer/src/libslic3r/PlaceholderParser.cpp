...
namespace client
{
    ...
    struct expr
    {
        ...
        {
#ifdef __ANDROID__
            std::string token(this->it_range.begin(), this->it_range.end());
            if (token.size() > 256)
                token.resize(256);
            std::replace(token.begin(), token.end(), '\n', '|');
            std::replace(token.begin(), token.end(), '\r', '|');
            __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative",
                "placeholder_expr_throw: %s token=%s", message, token.c_str());
#endif
            boost::throw_exception(qi::expectation_failure<Iterator>(
    ...
    struct MyContext : public ConfigOptionResolver {
        ...
        {
#ifdef __ANDROID__
            std::string token(it_range.begin(), it_range.end());
            if (token.size() > 256)
                token.resize(256);
            std::replace(token.begin(), token.end(), '\n', '|');
            std::replace(token.begin(), token.end(), '\r', '|');
            __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative",
                "placeholder_throw: %s token=%s", msg.c_str(), token.c_str());
#endif
        ...
        static void process_error_message(const MyContext *context, const boost::spirit::info &info, const Iterator &it_begin, const Iterator &it_end, const Iterator &it_error)
        {
            ...
            msg += "^\n";
#ifdef __ANDROID__
            std::string error_preview = error_line;
            if (error_preview.size() > 256)
                error_preview.resize(256);
            std::replace(error_preview.begin(), error_preview.end(), '\n', '|');
            std::replace(error_preview.begin(), error_preview.end(), '\r', '|');
            __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative",
                "placeholder_process_error: tag=%s line=%d pos=%zu line_text=%s",
                info.tag.c_str(), line_nr, size_t(error_pos), error_preview.c_str());
#endif
        }
    ...
    struct macro_processor : qi::grammar<Iterator, std::string(const MyContext*), qi::locals<bool>, skipper>
    {
        macro_processor() : macro_processor::base_type(start)
        {
            ...
            text_block = *(
                    ...
                    |   (lit('{') >> !(kw["elsif"] | kw["else"] | kw["endif"]) >> (macros(_r1)[_val += _1] > '}') | '}')
...
std::string PlaceholderParser::process(const std::string &templ, unsigned int current_extruder_id, const DynamicConfig *config_override, DynamicConfig *config_outputs, ContextData *context_data) const
{
    ...
    try {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "MobileSlicerNative",
            "placeholder_process_failed: extruder=%u error=%s template=%s",
            current_extruder_id, e.what(), preview.c_str());
#endif
        throw;
    }
}
