    ...
    std::vector<unsigned int>& printExtruders)
{
    ...
    int  extruder_nums = std::max<int>(1, print.config().nozzle_diameter.values.size());
    ...
    if (print.has_wipe_tower() && !print.get_fake_wipe_tower().outer_wall.empty()) {
        ...
        const FakeWipeTower& fake_wipe_tower = print.get_fake_wipe_tower();
        const Point wipe_tower_pos(scale_(fake_wipe_tower.pos[0]), scale_(fake_wipe_tower.pos[1]));
        const coord_t tower_margin = scale_(fake_wipe_tower.brim_width + 2.0f);
        const coord_t tower_max_x = wipe_tower_pos.x() + scale_(fake_wipe_tower.width) + tower_margin;
        const coord_t tower_max_y = wipe_tower_pos.y() + scale_(fake_wipe_tower.depth) + tower_margin;
        for (auto polyline : print.get_fake_wipe_tower().outer_wall.begin()->second) {
            polyline.remove_duplicate_points();
            expolyFromLines.emplace_back(polyline.points);
            const BoundingBox polyline_bounds(polyline.points);
            // MobileSlicer's Android Type1 wipe tower path uses Orca's real tower
            // generator, whose outer wall may already be stored in bed coordinates.
            // Orca's brim/skirt path expects local tower coordinates and translates
            // them by fake_wipe_tower.pos. Detect the absolute-coordinate case to
            // avoid applying the tower position twice, which pushes skirts outside
            // the printable area on small beds.
            const bool outline_is_already_positioned =
                polyline_bounds.min.x() >= wipe_tower_pos.x() - tower_margin &&
                polyline_bounds.min.y() >= wipe_tower_pos.y() - tower_margin &&
                polyline_bounds.max.x() <= tower_max_x &&
                polyline_bounds.max.y() <= tower_max_y;
            if (!outline_is_already_positioned) {
                expolyFromLines.back().translate(wipe_tower_pos);
            }
        }
    ...
    for (const PrintObject* object : print.objects()) {
        ...
        if (iter != objPrintVec.end()) {
            ...
            extruder_id = std::clamp(extruder_id, 0, extruder_nums - 1);
