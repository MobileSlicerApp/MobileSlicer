...
void PrintConfigDef::init_fff_params()
{
    ...
    {
        auto *opt = new ConfigOptionEnumsGeneric{ (int)Overhang_threshold_bridge };
        opt->keys_map = def->enum_keys_map;
    ...
    */
    ...
    def->nullable = true;
    {
        auto *opt = new ConfigOptionEnumsGenericNullable({ ntUndefine });
        opt->keys_map = def->enum_keys_map;
        def->set_default_value(opt);
    }
    ...
    def->mode = comAdvanced;
    {
        auto *opt = new ConfigOptionEnumsGeneric{ ZHopType::zhtSlope };
        opt->keys_map = def->enum_keys_map;
        def->set_default_value(opt);
    }
    ...
    def->mode = comAdvanced;
    {
        auto *opt = new ConfigOptionEnumsGeneric{ RetractLiftEnforceType::rletAllSurfaces };
        opt->keys_map = def->enum_keys_map;
        def->set_default_value(opt);
    }
    ...
    def->mode = comAdvanced;
    {
        auto *opt = new ConfigOptionEnumsGeneric{ ExtruderType::etDirectDrive };
        opt->keys_map = def->enum_keys_map;
        def->set_default_value(opt);
    }
    ...
    def->mode = comSimple;
    {
        auto *opt = new ConfigOptionEnumsGeneric{ NozzleVolumeType::nvtStandard };
        opt->keys_map = def->enum_keys_map;
        def->set_default_value(opt);
    }
...
//    def->min = 0;
    ...
    for (auto& opt_key : filament_extruder_override_keys) {
        ...
        case coEnums    : {
            auto *opt = new ConfigOptionEnumsGenericNullable(static_cast<const ConfigOptionEnumsGeneric*>(it_opt->second.default_value.get())->values);
            opt->keys_map = def->enum_keys_map;
            def->set_default_value(opt);
            break;
        }
        ...
        for (auto& key: key_set)
        {
            ...
            switch (optdef->type) {
                case coStrings:
                {
                    ...
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read string vector for %2%")%__LINE__%key;
                        continue;
                    }
                ...
                case coInts:
                {
                    ...
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read int vector for %2%")%__LINE__%key;
                        continue;
                    }
                ...
                case coFloats:
                {
                    ...
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read float vector for %2%")%__LINE__%key;
                        continue;
                    }
                ...
                case coPercents:
                {
                    auto* opt = dynamic_cast<ConfigOptionVector<double>*>(this->option(key));
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read percent vector for %2%")%__LINE__%key;
                        continue;
                    }
                    std::vector<double> new_values;
                ...
                case coFloatsOrPercents:
                {
                    ...
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read float-or-percent vector for %2%")%__LINE__%key;
                        continue;
                    }
                ...
                case coBools:
                {
                    ...
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read bool vector for %2%")%__LINE__%key;
                        continue;
                    }
                ...
                case coEnums:
                {
                    auto* opt = dynamic_cast<ConfigOptionVector<int>*>(this->option(key));
                    if (!opt) {
                        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(", Line %1%: can not read enum vector for %2%")%__LINE__%key;
                        continue;
                    }
                    std::vector<int> new_values;
...
static std::map<t_custom_gcode_key, t_config_option_keys> s_CustomGcodeSpecificPlaceholders{
    ...
    {"file_start_gcode",           {}},
    {"machine_start_gcode",         {}},
    {"machine_end_gcode",           {"layer_num", "layer_z", "max_layer_z", "filament_extruder_id", "timelapse_type"}},
    {"before_layer_change_gcode",   {"layer_num", "layer_z", "max_layer_z"}},
    {"layer_change_gcode",          {"layer_num", "layer_z", "max_layer_z", "most_used_physical_extruder_id", "curr_physical_extruder_id", "timelapse_type"}},
    {"timelapse_gcode",             {"layer_num", "layer_z", "max_layer_z", "most_used_physical_extruder_id", "curr_physical_extruder_id",
                                      "timelapse_pos_x", "timelapse_pos_y", "has_timelapse_safe_pos", "timelapse_type", "spiral_mode"}},
...
// change_filament_gcode
    ...
    new_def("most_used_physical_extruder_id", coInt, "Most-used physical extruder ID", "The most used physical extruder index used for the current tool-ordering phase.");
    new_def("curr_physical_extruder_id", coInt, "Current physical extruder ID", "The physical extruder index currently assigned to the active logical extruder.");
    new_def("timelapse_pos_x", coInt, "Timelapse safe position X", "The X coordinate chosen for safe timelapse capture.");
    new_def("timelapse_pos_y", coInt, "Timelapse safe position Y", "The Y coordinate chosen for safe timelapse capture.");
    new_def("has_timelapse_safe_pos", coBool, "Has timelapse safe position", "Indicates whether a safe timelapse capture position was found.");
    new_def("timelapse_type", coInt, "Timelapse type", "The active Orca timelapse mode for custom G-code conditionals.");
    new_def("spiral_mode", coBool, "Spiral vase", "Whether spiral vase mode is active for custom G-code conditionals.");
