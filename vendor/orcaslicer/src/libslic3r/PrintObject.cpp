...
PrintObject::PrintObject(Print* print, ModelObject* model_object, const Transform3d& trafo, PrintInstances&& instances) :
    ...
    m_trafo(trafo),
    // Seed the SoftFever label-object ID with a deterministic model-backed value.
    // If exclude-object support is later enabled, GCode::set_object_info() overwrites
    // this with its sequential export-time IDs before any related labels are emitted.
    m_id(model_object->id().id),
    // BBS
...
void PrintObject::bridge_over_infill()
{
    ...
    auto cmp_points_lexicographically = [](const Points &left, const Points &right) {
        const size_t count = std::min(left.size(), right.size());
        for (size_t idx = 0; idx < count; ++idx) {
            if (left[idx].x() != right[idx].x())
                return left[idx].x() < right[idx].x();
            if (left[idx].y() != right[idx].y())
                return left[idx].y() < right[idx].y();
        }
        return left.size() < right.size();
    };

    auto cmp_polygons_lexicographically = [&cmp_points_lexicographically](const Polygons &left, const Polygons &right) {
        const size_t count = std::min(left.size(), right.size());
        for (size_t idx = 0; idx < count; ++idx) {
            const BoundingBox left_bbox  = get_extents(Polygons{left[idx]});
            const BoundingBox right_bbox = get_extents(Polygons{right[idx]});
            if (left_bbox.min.x() != right_bbox.min.x())
                return left_bbox.min.x() < right_bbox.min.x();
            if (left_bbox.min.y() != right_bbox.min.y())
                return left_bbox.min.y() < right_bbox.min.y();
            if (left_bbox.max.x() != right_bbox.max.x())
                return left_bbox.max.x() < right_bbox.max.x();
            if (left_bbox.max.y() != right_bbox.max.y())
                return left_bbox.max.y() < right_bbox.max.y();
            if (left[idx].area() != right[idx].area())
                return left[idx].area() < right[idx].area();
            if (cmp_points_lexicographically(left[idx].points, right[idx].points))
                return true;
            if (cmp_points_lexicographically(right[idx].points, left[idx].points))
                return false;
        }
        return left.size() < right.size();
    };

    auto candidate_surface_less = [&cmp_polygons_lexicographically, &cmp_points_lexicographically](const CandidateSurface &left, const CandidateSurface &right) {
        if (left.layer_index != right.layer_index)
            return left.layer_index < right.layer_index;

        BoundingBox left_bbox  = get_extents(left.new_polys);
        BoundingBox right_bbox = get_extents(right.new_polys);
        if (left_bbox.min.x() != right_bbox.min.x())
            return left_bbox.min.x() < right_bbox.min.x();
        if (left_bbox.min.y() != right_bbox.min.y())
            return left_bbox.min.y() < right_bbox.min.y();
        ...
        const double left_area  = area(left.new_polys);
        const double right_area = area(right.new_polys);
        if (left_area != right_area)
            return left_area < right_area;

        if (cmp_polygons_lexicographically(left.new_polys, right.new_polys))
            return true;
        if (cmp_polygons_lexicographically(right.new_polys, left.new_polys))
            return false;

        return cmp_points_lexicographically(left.original_surface->expolygon.contour.points,
                                            right.original_surface->expolygon.contour.points);
    };
    ...
    // SECTION to gather and filter surfaces for expanding, and then cluster them by layer
    {
        ...
        std::vector<CandidateSurface> candidate_surfaces_sorted(candidate_surfaces.begin(), candidate_surfaces.end());
        std::sort(candidate_surfaces_sorted.begin(), candidate_surfaces_sorted.end(), candidate_surface_less);

        for (const CandidateSurface &c : candidate_surfaces_sorted) {
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, clustered_layers_for_threads.size()), [po = static_cast<const PrintObject *>(this),
                                                                                           ...
                                                                                           &cmp_points_lexicographically,
                                                                                           determine_bridging_angle,
                                                                                           &candidate_surface_less,
        ...
        for (size_t cluster_idx = r.begin(); cluster_idx < r.end(); cluster_idx++) {
            for (size_t job_idx = 0; job_idx < clustered_layers_for_threads[cluster_idx].size(); job_idx++) {
                ...
                std::stable_sort(surfaces_by_layer[lidx].begin(), surfaces_by_layer[lidx].end(),
                                 [&candidate_surface_less](const CandidateSurface &left, const CandidateSurface &right) {
                                     ...
                                     return candidate_surface_less(left, right);
                                 });
                if (surfaces_by_layer[lidx].size() > 2) {
                    ...
                    std::stable_sort(surfaces_by_layer[lidx].begin() + 1, surfaces_by_layer[lidx].end(),
                                     [origin, &candidate_surface_less](const CandidateSurface &left, const CandidateSurface &right) {
                                         auto a = get_extents(left.new_polys);
                                         auto b = get_extents(right.new_polys);

                                         const double left_distance  = (origin - a.min.cast<double>()).squaredNorm();
                                         const double right_distance = (origin - b.min.cast<double>()).squaredNorm();
                ...
                for (const CandidateSurface &candidate : surfaces_by_layer[lidx]) {
                    ...
                    std::sort(limiting_area.begin(), limiting_area.end(),
                              [&cmp_points_lexicographically](const Polygon &left, const Polygon &right) {
                                  return cmp_points_lexicographically(left.points, right.points);
                              });
                    ...
                    {
                        const CandidateSurface *deterministic_overlap = nullptr;
                        ...
                        for (const CandidateSurface &s : expanded_surfaces) {
                            if (intersection(s.new_polys, tmp_expanded_area).empty())
                                continue;
                            if (deterministic_overlap == nullptr || candidate_surface_less(s, *deterministic_overlap))
                                deterministic_overlap = &s;
                        }
                        if (deterministic_overlap != nullptr) {
                            bridging_angle = deterministic_overlap->bridge_angle;
                    ...
                    std::sort(bridging_area.begin(), bridging_area.end(),
                              [&cmp_points_lexicographically](const Polygon &left, const Polygon &right) {
                                  return cmp_points_lexicographically(left.points, right.points);
                              });
                    ...
                    std::sort(expansion_area.begin(), expansion_area.end(),
                              [&cmp_points_lexicographically](const Polygon &left, const Polygon &right) {
                                  return cmp_points_lexicographically(left.points, right.points);
                              });
