...
size_t ViewerImpl::load_layer_range(const GCodeInputData& gcode_data, long min_layer, long max_layer, size_t max_vertices)
{
    ...
    const bool indexed_layers_available = layer_filter_enabled && !gcode_data.layer_vertex_ranges.empty();
    if (indexed_layers_available) {
        const long capped_min_layer = std::max<long>(0, min_layer);
        const long capped_max_layer = std::min<long>(max_layer, static_cast<long>(gcode_data.layer_vertex_ranges.size()) - 1);
        if (capped_max_layer >= capped_min_layer) {
            for (long layer_id = capped_min_layer; layer_id <= capped_max_layer; ++layer_id) {
                const LayerVertexRange& range = gcode_data.layer_vertex_ranges[static_cast<size_t>(layer_id)];
                selected_vertices += range.end - range.begin;
    ...
    if (indexed_layers_available) {
        ...
        for (long layer_id = capped_min_layer; layer_id <= capped_max_layer; ++layer_id) {
            ...
            for (size_t index = range.begin; index < range.end; ++index) {
                PathVertex vertex = gcode_data.vertices[index];
