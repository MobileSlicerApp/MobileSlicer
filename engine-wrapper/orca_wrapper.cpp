...
static void apply_json_overrides(const std::string& json, Slic3r::DynamicPrintConfig& config)
{
    ...
    if (const auto value = extract_number(json, "brim_width")) {
        ...
        // following the user-specified width directly. Keep legacy width-only semantics
        // only when the profile does not now own brim_type explicitly.
        if (!extract_string(json, "brim_type")) {
...
static void apply_model_object_overrides(const std::string& json, Slic3r::Model& model)
{
    if (json.empty()) {
        return;
    }
...
static Slic3r::Calib_Params extract_calibration_params(const std::string& json)
{
