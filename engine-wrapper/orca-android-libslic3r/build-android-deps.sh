...
DEPS_BUILD="$PROJECT_ROOT/engine-wrapper/orca-android-libslic3r/android-deps-build"
DEPS_INSTALL="$PROJECT_ROOT/engine-wrapper/orca-android-libslic3r/android-deps-install"
ORCA_DEPS_SRC="$PROJECT_ROOT/vendor/orcaslicer/deps"
...
build_gmp() {
    ...
    local install_dir="$DEPS_INSTALL/gmp"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://ftp.gnu.org/gnu/gmp/gmp-6.3.0.tar.xz | tar xJ --strip-components=1
    fi
...
build_mpfr() {
    ...
    local install_dir="$DEPS_INSTALL/mpfr"
    local gmp_dir="$DEPS_INSTALL/gmp"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://ftp.gnu.org/gnu/mpfr/mpfr-4.2.1.tar.xz | tar xJ --strip-components=1
    fi
...
build_openssl() {
    ...
    local install_dir="$DEPS_INSTALL/openssl"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://github.com/openssl/openssl/releases/download/openssl-3.1.7/openssl-3.1.7.tar.gz | tar xz --strip-components=1
    fi
...
build_boost() {
    ...
    local install_dir="$DEPS_INSTALL/boost"
    local headers_dir="$DEPS_INSTALL/boost-headers"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://github.com/boostorg/boost/releases/download/boost-1.84.0/boost-1.84.0-cmake.tar.xz | tar xJ --strip-components=1
    fi
    ...
    cmake --build . -j"$NPROC" --target boost_filesystem boost_thread boost_iostreams boost_regex boost_date_time boost_atomic boost_chrono boost_nowide
    cmake --install .
    ...
    rm -rf "$headers_dir"
    mkdir -p "$headers_dir"
    find "$src_dir/libs" -type d -name "include" | while read inc_dir; do
...
build_tbb() {
    ...
    local install_dir="$DEPS_INSTALL/tbb"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://github.com/oneapi-src/oneTBB/archive/refs/tags/v2021.11.0.tar.gz | tar xz --strip-components=1
    fi
...
build_nlopt() {
    ...
    local install_dir="$DEPS_INSTALL/nlopt"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://github.com/stevengj/nlopt/archive/v2.5.0.tar.gz | tar xz --strip-components=1
    fi
...
build_cgal() {
    ...
    local install_dir="$DEPS_INSTALL/cgal"
    ...
    if [ ! -d "$src_dir" ]; then
        mkdir -p "$src_dir"
        cd "$src_dir"
        curl -sL https://github.com/CGAL/cgal/releases/download/v5.6.1/CGAL-5.6.1.tar.xz | tar xJ --strip-components=1
    fi
...
echo "Building all Android x86_64 dependencies..."
echo "NDK: $NDK_ROOT"
...
echo "GMP:     $DEPS_INSTALL/gmp"
echo "MPFR:    $DEPS_INSTALL/mpfr"
echo "OpenSSL: $DEPS_INSTALL/openssl"
echo "Boost:   $DEPS_INSTALL/boost (headers: $DEPS_INSTALL/boost-headers)"
echo "TBB:     $DEPS_INSTALL/tbb"
echo "NLopt:   $DEPS_INSTALL/nlopt"
echo "CGAL:    $DEPS_INSTALL/cgal"
